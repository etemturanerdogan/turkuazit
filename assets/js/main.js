...
function initUserDropdown() {
    ...
    const dropdownItems = userMenu.querySelectorAll('.site-header__dropdown-item[href*="#"]');
...
function initSlidingPanels() {
    ...
    tabButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // ─────────────────────────────────────────────────────────────
            const slideDirection = targetPanelIndex > currentPanelIndex ? 'left' : 'right';
            ...
            const targetPanel = document.querySelector(`[data-panel-id="${targetPanelId}"]`);
            ...
            if (container) {
                ...
                container.style.minHeight = Math.max(currentHeight, targetHeight) + 'px';
            }
            ...
            setTimeout(() => {
                ...
                currentPanel.classList.remove('is-active', 'slide-out-left', 'slide-out-right');
    ...
    function goToPanel(targetPanelId) {
        const targetBtn = document.querySelector(`.panel-tab-btn[data-panel="${targetPanelId}"]`);
        if (targetBtn) {
            targetBtn.click();
        }
    }
...
function initSubPanels() {
    ...
    document.querySelectorAll('.sub-panel-tab').forEach(tab => {
        tab.addEventListener('click', (e) => {
            ...
            const targetSubPanel = panelCard.querySelector(`[data-sub-panel-id="${targetSubPanelId}"]`);
            if (targetSubPanel) {
    ...
    document.querySelectorAll('[data-sub-panel-goto]').forEach(el => {
        el.addEventListener('click', (e) => {
            ...
            if (groupName) {
                ...
                const tabContainer = document.querySelector(`[data-sub-panel-group="${groupName}"]`);
                if (tabContainer) {
                    targetTab = tabContainer.querySelector(`[data-sub-panel="${targetSubPanelId}"]`);
                }
            } else {
                // Grup adı yoksa en yakın panel kartındaki sekmeyi bul
                const panelCard = el.closest('.panel-card');
                if (panelCard) {
                    targetTab = panelCard.querySelector(`[data-sub-panel="${targetSubPanelId}"]`);
                }
            }
